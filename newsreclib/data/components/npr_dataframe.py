...
class NPRDataFrame(Dataset):
    ...
    def __init__(
        ...
        self.dst_dir = os.path.join(self.data_dir, "npr_" + self.dataset_size)
        self.dst_dir_stage = os.path.join(
            self.data_dir, "npr_" + self.dataset_size, self.data_split
        )
    ...
    def _load_news(self) -> pd.DataFrame:
        ...
        stage_dir = os.path.join(self.data_dir, "npr_" + self.dataset_size, self.data_split)
        ...
        if file_utils.check_integrity(parsed_news_filepath):
            ...
            for stage in ["train", "dev", "test"]:
                stage_dir = os.path.join(self.data_dir, "npr_" + self.dataset_size, stage)
                parsed_news_filepath = os.path.join(stage_dir, "parsed_news.tsv")
    ...
    def _load_behaviors(self) -> pd.DataFrame:
        ...
        if file_utils.check_integrity(parsed_behaviors_filepath):
            ...
            if not file_utils.check_integrity(parsed_behaviors_filepath):
                ...
                # )
                df_beh = pd.read_parquet(os.path.join(self.data_dir, "behaviors.parquet") )
                ...
                def convert_to_nid_single(row):
                    return [nid2index[row]]

                
                print(df_beh)
                df_beh["history"] = df_beh["history"].apply(convert_to_nid)
                df_beh["impressions"] = df_beh["impressions"].apply(convert_to_nid)
                df_beh["clickedItem"] = df_beh["clickedItem"].apply(convert_to_nid_single)
                # df_beh["article_ids_inview"] = df_beh["article_ids_inview"].apply(convert_to_nid)
                ...
                # )
                df_beh["formatted_binary_label"] = df_beh.apply(
                    lambda row: " ".join(
                        [f"{article}-1" for article in row["clickedItem"]] +
                        [f"{article}-0" for article in row["impressions"] if article not in row["clickedItem"]]
                    ), 
                    axis=1
                )

                df_beh = df_beh[['userId', 'history', 'formatted_binary_label']]
