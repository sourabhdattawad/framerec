...
def unzip_file(zip_path):
    extract_path = os.path.dirname(zip_path)  # Extracts in the same folder as the zip file
...
class EbnerdDataFrame(Dataset):
    ...
    def __init__(
        ...
        self.dst_dir = os.path.join(self.data_dir, "ebnerd_" + self.dataset_size)
        self.dst_dir_stage = os.path.join(
            self.data_dir, "ebnerd_" + self.dataset_size, self.data_split
        )
        if not os.path.isdir(self.dst_dir_stage):
            os.makedirs(self.dst_dir_stage)

        self.ebnerd_gzip_filename = "ebnerd_" + self.dataset_size + ".zip"

        if download:
            url = dataset_url[dataset_size]
            log.info(f"Downloading Ebnerd {self.dataset_size} dataset from {url}.")
            try:
                data_utils.download_and_extract_dataset(
                    data_dir=self.data_dir,
                    url=url,
                    filename=self.ebnerd_gzip_filename,
                ...
                log.warn(
                    f"Downloading Ebnerd {self.dataset_size} dataset from {url} failed due to error: {e}. Download the dataset manually and try again."
                )
            if not self.use_plm or self.use_pretrained_categ_embeddings:
    ...
    def _load_news(self) -> pd.DataFrame:
        ...
        stage_dir = os.path.join(self.data_dir, "ebnerd_" + self.dataset_size, self.data_split)
        parsed_news_filepath = os.path.join(stage_dir, "parsed_news.tsv")

        if file_utils.check_integrity(parsed_news_filepath):
            # news already parsed
            log.info(f"News already parsed. Loading from {parsed_news_filepath}.")
            ...
            unzip_file(os.path.join(self.data_dir, self.ebnerd_gzip_filename))
            print(self.dst_dir_stage)
            raw_news_filepath = os.path.join(self.data_dir, "articles.parquet")

            news_filepath = os.path.join(self.dst_dir, "news_title.tsv")
            
            if not file_utils.check_integrity(news_filepath):
                ...
                news_title, news_category, news_subcategory, nid2index = self._process_news_files(
                    os.path.join(raw_news_filepath)
                )
            ...
            filepath = os.path.join(self.dst_dir, self.data_split, "news.tsv")
            news = pd.read_table(
            ...
            for stage in ["train", "dev", "test"]:
                stage_dir = os.path.join(self.data_dir, "ebnerd_" + self.dataset_size, stage)
                parsed_news_filepath = os.path.join(stage_dir, "parsed_news.tsv")
    ...
    def _load_behaviors(self) -> pd.DataFrame:
        ...
        #     self.dst_dir_stage, self.data_split, "parsed_behaviors_" + str(self.seed) + ".tsv"
        # )
        stage_dir = os.path.join(self.data_dir, "ebnerd_" + self.dataset_size, self.data_split)
        parsed_behaviors_filepath = os.path.join(stage_dir, "parsed_behaviors.tsv")

        if file_utils.check_integrity(parsed_behaviors_filepath):
            # behaviors data already parsed
            log.info(f"User behaviors data already parsed. Loading from {parsed_behaviors_filepath}.")
            behaviors = pd.read_table(
                filepath_or_buffer=parsed_behaviors_filepath,
                converters={
            ...
            raw_behaviors_filepath = os.path.join(
                self.dst_dir, self.data_split , "behaviors_" + str(self.seed) + ".tsv"
            )
            # parsed_behaviors_file = os.path.join(self.dst_dir_stage, "parsed_behaviors.tsv")
            stage_dir = os.path.join(self.data_dir, "ebnerd_" + self.dataset_size, self.data_split)
            parsed_behaviors_filepath = os.path.join(stage_dir, "parsed_behaviors.tsv")

            if not file_utils.check_integrity(parsed_behaviors_filepath):
                ...
                def convert_to_nid(row):
                    res = []
                    for i in row:
                        res.append(nid2index[i])
                    return res
                
                merged_df["article_id_fixed"] = merged_df["article_id_fixed"].apply(convert_to_nid)
                merged_df["article_ids_clicked"] = merged_df["article_ids_clicked"].apply(convert_to_nid)
                merged_df["article_ids_inview"] = merged_df["article_ids_inview"].apply(convert_to_nid)
                # merged_df["formatted_binary_label"] = merged_df.apply(
                #     lambda row: " ".join([f"{article}-{1 if article in row['article_ids_clicked'] else 0}" for article in row["article_ids_inview"]]), axis=1
                # )
                merged_df["formatted_binary_label"] = merged_df.apply(
                    lambda row: " ".join(
                        [f"{article}-1" for article in row["article_ids_clicked"]] +
                        [f"{article}-0" for article in row["article_ids_inview"] if article not in row["article_ids_clicked"]]
                    ), 
                    axis=1
                )


                merged_df = merged_df[['user_id', 'article_id_fixed', 'formatted_binary_label']]
            ...
            behaviors["uid"] = behaviors["uid"].apply(lambda x: str(x))
            ...
            log.info(
                f"Caching parsed behaviors of size {len(behaviors)} to {parsed_behaviors_filepath}."
            )
           
            stage_dir = os.path.join(self.data_dir, "ebnerd_" + self.dataset_size, self.data_split)
            parsed_news_filepath = os.path.join(stage_dir, "parsed_behaviors.tsv")
            
            file_utils.to_tsv(behaviors, parsed_news_filepath)
